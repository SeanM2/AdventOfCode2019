...
def InputFile():
    ...
    for row in fileContent:
        outputContent.append(max(int(row),0))

    return outputContent

def CalculateFuelRequiredByMass(inputMass):
    outputMass = (inputMass/3) # divide by 3
    outputMass = math.floor(outputMass) # round down
    outputMass = outputMass - 2 # subtract 2

    return outputMass

def main():
    inputFileContent = InputFile()
    totalRequiredFuel = 0
    requiredFuel = 0
    previousStepRequiredFuel = 1

    while previousStepRequiredFuel > 0:
        requiredFuel = 0
        for i in range(len(inputFileContent)):
            fuelMass = max(CalculateFuelRequiredByMass(inputFileContent[i]),0)
            requiredFuel += fuelMass
            inputFileContent[i] = fuelMass
        
        totalRequiredFuel += requiredFuel
        print(requiredFuel)

        previousStepRequiredFuel = requiredFuel

    print(totalRequiredFuel)    
